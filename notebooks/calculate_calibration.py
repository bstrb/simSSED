...
def calculate_calibration(
    ...
    calibration_viljar = 1 / 3.9 / 183
    # Compute the scattering vector q (1/Å per pixel) using q = 4π sin(θ) / λ
    # Here wavelength_A is used directly to get q in 1/Å
    q_per_pixel = (4 * np.pi * np.sin(theta)) / wavelength_A

    print("Calibration (1/Å per pixel) =", calibration)
    print("Scattering vector q (1/Å per pixel) =", q_per_pixel)

    return calibration, calibration_viljar, q_per_pixel

# Example usage:
if __name__ == "__main__":
    # Define experimental parameters:
    wavelength_A = 1.5406        # Cu Kα X-ray wavelength in Å
    camera_length_m = 2.0        # camera length in meters
    pixel_resolution = 1.0 / 55e-6  # Example: 55 µm pixel size -> pixels per meter

    # Call the function with these parameters
    calibration_value, calibration_viljar, q_value = calculate_calibration(
    )

    print(f"Calculated calibration: {calibration_value:.4f} 1/Å per pixel")
    print(f"Viljar calibration: {calibration_viljar:.4f} 1/Å per pixel")
    print(f"Calculated scattering vector q: {q_value:.4f} 1/Å per pixel")
