...
def process_stream_file(stream_file_path, metric_weights=None):
    ...
    The raw metrics extracted are:
      - weighted_rmsd, length_deviation, angle_deviation, peak_ratio, percentage_indexed
              ...
              - results is a list of tuples containing raw metric values and the chunk content.
                Each valid result has the form:
                    (stream_file_name, event_number, weighted_rmsd, length_deviation,
                     angle_deviation, peak_ratio, percentage_indexed, chunk_content)
    ...
    """
    metric_names = [
        'weighted_rmsd', 'percent_indexed', 'length_deviation', 'angle_deviation', 'peak_ratio', 'percentage_indexed'  # New metric added
    ]
    ...
    if isinstance(metric_weights, (list, tuple)) and len(metric_weights) == len(metric_names):
        ...
        metric_weights = {
            ...
            'angle_deviation': 1,
            'peak_ratio': 1,
            'percentage_indexed': -1  # Negative weight to favor higher percentages
        }
    ...
    with open(stream_file_path, 'r') as file:
        ...
        for chunk in tqdm(chunks, desc=f"Processing chunks in {os.path.basename(stream_file_path)}", unit="chunk"):
            ...
            # peak_ratio, percentage_indexed, chunk_content)
            data = extract_chunk_data(chunk, original_cell_params)
            if data is None:
                continue
            (event_number, weighted_rmsd, percent_indexed, length_deviation, angle_deviation, peak_ratio, percentage_indexed, chunk_content) = data
            
            if event_number is not None:
                # Only add valid chunks (all required metrics not None)
                if None not in (weighted_rmsd, percent_indexed, length_deviation, angle_deviation, peak_ratio, percentage_indexed):
                    results.append((
...
def process_all_stream_files(folder_path, metric_weights=None, normalization_method='zscore'):
    ...
    # 5: peak_ratio, 6: percentage_indexed, 7: chunk_content

    global_metrics = {
        'weighted_rmsd': [r[2] for r in valid_results],
        'percent_indexed': [r[3] for r in valid_results],
        ...
        'percentage_indexed': [r[7] for r in valid_results]
    }
    ...
    if normalization_method == 'minmax':
        ...
        for key, values in global_metrics.items():
            if key == 'percentage_indexed' or key == 'percent_indexed':
    ...
    metric_names = [
        'weighted_rmsd', 'percent_indexed', 'length_deviation', 'angle_deviation', 'peak_ratio', 'percentage_indexed'
    ]
    ...
    for i, r in enumerate(valid_results):
        ...
        norm_percent_indexed = norm_metrics['percent_indexed'][i]
        ...
        norm_percentage_indexed = norm_metrics['percentage_indexed'][i]
        ...
        updated_results.append((
            ...
            combined_metric,
            norm_weighted_rmsd,
            norm_percent_indexed,
            norm_length_deviation,
            norm_angle_deviation,
            norm_peak_ratio,
            norm_percentage_indexed,
            r[8]  # chunk content
        ))
    ...
    csv_header = [
        'stream_file', 'event_number', 'combined_metric', 'weighted_rmsd', 'percent_indexed', 'length_deviation', 'angle_deviation', 'peak_ratio', 'percentage_indexed'
    ]
    with open(output_csv_path, mode='w', newline='') as csv_file:
        ...
        for event, group in groupby(updated_results, key=lambda x: x[1]):
            ...
            csv_writer.writerow([f"Event: {event}"] + [""] * (len(csv_header)-1))
            for row in group:
