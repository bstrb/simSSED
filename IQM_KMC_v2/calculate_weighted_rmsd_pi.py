...
def calculate_weighted_rmsd_pi(fs_ss, intensities, ref_fs_ss, tolerance_factor=2.0):
    # Ensure inputs are non-empty
    if len(fs_ss) == 0 or len(ref_fs_ss) == 0:
        print("Warning: Empty input for peaks or reflections.")
        return float('inf'), 0.0  # Returning (RMSD, percent_indexed)
    ...
    length_outliers = len(distances) - np.sum(inliers)
    # Convert to percentage by multiplying by 100
    percent_indexed = (np.sum(inliers) / len(distances)) 
    # print(f"Number of outliers: {length_outliers}")
    # print(f"Percentage indexed: {percent_indexed}%")

    if not np.any(inliers):  # Check if there are any inliers
        print("Warning: No inliers found.")
        return float('inf'), percent_indexed
    ...
    # Return a tuple: (weighted RMSD, percentage indexed)
    return weighted_rmsd, percent_indexed
