...
def _calculate_excitation_error(
    ...
    r = 1 / wavelength
    # Ensure the optical axis vector is contiguous
    u = np.ascontiguousarray(optical_axis_vector)
    # Ensure recip is contiguous
    o = np.ascontiguousarray(recip)
    c = r * u
    diff = o - c
    # Make sure diff is contiguous (usually it is if o and c are)
...
def _get_intersection_with_ewalds_sphere_with_precession(
    ...
    # Compute the cylindrical coordinate rho
    # First compute np.dot(o, u) on contiguous arrays
    # The [:, np.newaxis] reshaping is fine after the dot product.
    rho = np.sum((np.dot(o, u)[:, np.newaxis] * u - o) ** 2, axis=1) ** 0.5
    a = np.deg2rad(precession_angle)
    first_half = r * np.cos(a) - r + np.sqrt(r**2 - rho**2)
    upper = first_half - np.sqrt(r**2 - (r * np.sin(a) + rho) ** 2)
    lower = first_half - np.sqrt(r**2 - (r * np.sin(a) - rho) ** 2)
    intersection = (d < (upper + max_excitation_error)) & (
        d > (lower - max_excitation_error)
    )
    return intersection, excitation_error