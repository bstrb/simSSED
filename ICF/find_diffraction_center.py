...
#         #     print(f"Convergence reached with center found {center} .")
#         #     break
#         if shift_mag < threshold:
...
def find_diffraction_center(image, mask, threshold=0.01, max_iters=10, step_size=1.0, n_steps=3, 
                            n_wedges=8, n_rad_bins=200, plot_profiles=False, frame_number=None):
    center = center_of_mass_initial_guess(image, mask)
    indices = np.indices(image.shape)  # Precompute indices
    
    for iteration in range(max_iters):
        dx, dy = find_best_center_shift(
            image, mask, center, indices,
            step_size=step_size,
            n_steps=n_steps,
            n_wedges=n_wedges,
            n_rad_bins=n_rad_bins
        ...
        if shift_mag < threshold:
            ...
            if frame_number is not None:
                print(f"Frame {frame_number}: Convergence reached with center found {center}.")
            else:
                print(f"Convergence reached with center found {center}.")
            break
